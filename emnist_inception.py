...
img_size = 150  # Уменьшенный размер изображения для MobileNet вместо Inception
...
# ВАЖНО: Используем меньший размер для экономии памяти
        ...
        for phase in ['train', 'test']:
            ...
            for inputs, labels in tqdm(dataloader):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    # MobileNet не имеет вспомогательных выходов, как Inception
...
# Используем MobileNet вместо Inception для экономии памяти
def initialize_mobilenet():
    # Используем MobileNet вместо Inception v3, так как она требует гораздо меньше памяти
...
print(f"\nОбучение модели: mobilenet")

# Инициализация модели MobileNet
...
model_name = "inception"  # Сохраняем под тем же именем для совместимости с сравнением
# Сохранение модели
...
plt.title(f'Функция потерь модели MobileNet (замена Inception)')
plt.xlabel('Эпоха')
plt.ylabel('Потери')
plt.legend()
...
plt.title(f'Точность модели MobileNet (замена Inception)')
plt.xlabel('Эпоха')
plt.ylabel('Точность')
plt.legend()
...
print(f"\nРезультаты для модели MobileNet (замена Inception):")
...
print(f"\nОбучение модели MobileNet (замена Inception) завершено. Результаты сохранены в {model_name}_results.png и {model_name}_history.json")
print("\nПримечание: Использована MobileNet вместо Inception v3 для экономии памяти CUDA")
